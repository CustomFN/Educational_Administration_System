					        ...
					        <tbody>
					        	...
					        	<c:forEach items="${pageInfo.list }" var="teacher">
					        		 <tr>
										...
										<th>${teacher.sex }</th>
										<th><fmt:formatDate pattern="yyyy-MM-dd" value="${teacher.birthyear }" /></th>
										<th>${teacher.degree }</th>
										<th>${teacher.title }</th>
										<th><fmt:formatDate pattern="yyyy-MM-dd" value="${teacher.grade }" /></th>
										<th>${teacher.collegename }</th>
										<th>
											<button class="btn btn-default btn-xs btn-info" onClick="location.href='/admin/editTeacher?id=${teacher.userid}'">修改</button>
										<button class="btn btn-default btn-xs btn-danger btn-primary" onClick="location.href='/admin/removeTeacher?id=${teacher.userid}'">删除</button>
										</th>
						            </tr>
					        	</c:forEach>
